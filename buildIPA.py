...
tempFinder = None
#git地址
gitPath = "http://gitlab.enn.cn/iOS-Proj/iCom_iOS.git"
#checkout后的本地路径
target_path = commendPath + "Documents"
#主路径
mainPath = None
# 指定项目下编译目录
build_path = "build"#证书名
certificateName = "iPhone Developer:Huang Jing(********)"
#firToken
firToken = "e379401c748bf21********13752cf961"
#邮件参数
emailFromUser = "zhangrongwuios@icloud.com"
emailToUser = "zhangrongwu@sina.com"
emailPassword = "********"
emailHost = "184.31.150.55"



#上传到自定义服务器参数
host = "10.37.149.160"#主机
port = 8080 #端口
username = "appadmin" #用户名
password = "********" #密码
#最终ipa存放位置------暂时这么写----正确写法不能除了target_path不应该有绝对路径的
local = "/Users/zhangrongwu/Documents/bea23b709e7ef08********b20f1e787/ICome.ipa" #本地文件或目录，与远程一致，
remote = "/data/tomcat8/webapps/ios"#远程文件或目录，与本地一致，当前为linux目录格式
...
def buildApp():
    ...
    if isWorkSpace:
       # os.system("cd %s;xcodebuild -workspace %s.xcworkspace -scheme %s CODE_SIGN_IDENTITY='%s' -configuration %s CONFIGURATION_BUILD_DIR=%s/%s.xcworkspace ONLY_ACTIVE_ARCH=NO"%(mainPath,targetName,targetName,certificateName,build_config,mainPath,targetName))
        print("build release start")
       #大方支援的两行
        os.system ('xcodebuild -list')
        os.system ('cd %s;xcodebuild -workspace %s.xcworkspace  -scheme %s -configuration release -derivedDataPath %s ONLY_ACTIVE_ARCH=NO || exit' % (mainPath,targetName,targetName,build_path))
    else:
...
def cerateIPA():
    ...
    os.system ("cd %s;xcrun -sdk iphoneos PackageApplication -v %s/Build/Products/Release-iphoneos/%s.app -o %s/%s.ipa CODE_SIGN_IDENTITY='%s'"%(mainPath,mainPath,targetName,mainPath,targetName,certificateName))
    return
...
def scp_uploadFile():
    cmd='scp %s appadmin@%s:%s'%(local,host,remote)
...
def main():
    ...
    httpAddress = uploadToFir()
     #上传到服务器
    #sftp_uploadFile() #连接被拒 ： 根据不同的服务器有不同的上传方法
    scp_uploadFile()
    #发邮件给测试
